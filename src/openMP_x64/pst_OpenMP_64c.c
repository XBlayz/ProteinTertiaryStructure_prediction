...
type energy(char* s, int n, VECTOR phi, VECTOR psi) {
	// Calcolo vettore delle coordinate del "Backbone" & "Rama Energy"
	MATRIX coords = NULL;
	type rama = 0.0;
	//* OpenMP (SCARTATA)
	...
	// {
	//	 #pragma omp section
			coords = backbone(n, phi, psi);
	// 	 #pragma omp section
			rama = rama_energy(n, phi, psi);
	// }

	// Calcolo delle "H. Energie", "E. Energie" & "P. Energie"
	type hydro = 0.0;
	type elec = 0.0;
	type pack = 0.0;
	//* OpenMP (SCARTATA)
	...
	// {
	//	 #pragma omp section
			hydro = hydrophobic_energy(s, n, coords);
	//	 #pragma omp section
			elec = electrostatic_energy(s, n, coords);
	//	 #pragma omp section
			pack = packing_energy(s, n, coords);
	// }
